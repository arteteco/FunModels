...
# How many thin and samples? If the dataset is kinda large go for 250, otherwise 500

thinSampl = 300

# END SETUP for me, but the rest should still be customized for your needs

alldat = read.csv(paste(fam,".csv", sep=""))

alldat <- drop_na(alldat)

# Where should it save the RData once the MCMC is done?

rdat<- paste("HMSC/Hmsc", fam, ".RData", sep="")

# Where should it save the beta file

betaOut <- paste("HMSC_beta/HMSC_Beta_", fam, ".csv", sep="")


# Where should it save the final results

finalResult <- paste("omegaCor/HMSC_OmegaCor_", fam, ".csv", sep="")
...
Y = select(alldat, starts_with(fam))
